...
public class ResPartner extends OModel {
	...
	OColumn is_company = new OColumn("Is Appointment", OBoolean.class)
			.setDefault(false);
	...
	OColumn mobile = new OColumn("Patient Mobile", OText.class);
	OColumn email = new OColumn("Email", OText.class);
	OColumn company_id = new OColumn("Company", ResHospital.class,
			RelationType.ManyToOne).addDomain("is_company", "=", true);

	// Extra Demo Module Columns

	OColumn date = new OColumn("Appointment Date", ODateTime.class);
	OColumn title = new OColumn("Title", ResPartnerTitle.class,
			RelationType.ManyToOne);
	OColumn parent_id = new OColumn("Related Appointment", ResPartner.class,
			RelationType.ManyToOne).addDomain("is_company", "=", true);
	...
	OColumn customer = new OColumn("is Patient", OBoolean.class);
	OColumn supplier = new OColumn("is Prescription", OBoolean.class);
	OColumn state_id = new OColumn("State", ResCountry.class,
			RelationType.ManyToOne);
	OColumn country_id = new OColumn("Country", ResCountry.class,
			RelationType.ManyToOne);
    //OColumn family = new OColumn("Family", Family.class, OColumn.RelationType.ManyToOne);

    //Extra for Patients

    //Extra for Medicaments

    //Extra for Prescription

     OColumn physician_id = new OColumn("Prescribing Doctor", ResPartner.class,
            OColumn.RelationType.ManyToOne).addDomain("supplier","=",true);


    //  OColumn template = new OColumn("Medication ")
    OColumn medicament_id = new OColumn("Medicament", ResPartner.class,
            OColumn.RelationType.ManyToOne).addDomain("supplier","=",true);
